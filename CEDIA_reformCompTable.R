...
# Update: Sr Legacy 
tc_sr <- tc[tc$fuente_==1,c("id_srleg","fuente_","IngredientesCodigo")]
tc_sr <- merge(tc_sr,sr,by.x="id_srleg",by.y="food_id",all.x=T)

# Update V4 Tcomp  
tc_t4 <- tc[tc$fuente_==2,c("cd_t4","fuente_","IngredientesCodigo","id_srleg")]
tc_t4 <- merge(tc_t4,t4[,-grep("fuente",names(t4))],by="cd_t4", all.x=T)
